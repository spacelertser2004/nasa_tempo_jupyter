...
trop_scr_dir = 'C:/Users/mlertsmi/Desktop/TEMPO Mission/merge/nasa-airathon-merge/tropomi_data/secure.txt'
trop_secure = dict([e.split('=') for e in open(trop_scr_dir, 'r').read().split('\n')])
...
def loadFileS3(row):
    for i in range(1):
        try:
            values = {'email' : trop_secure['username'], 'passwd' : trop_secure['password'], 'action' : 'login'}
...
def pullTropomi(row, fine = True):
    ...
    try:     
        os.remove(file) 
        print(f'{file} Successfuly Deleted')
    except:
